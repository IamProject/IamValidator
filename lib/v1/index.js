...
function _callbackChain(done, [f, ...rest]) {
  if (!f) {
    done();
...
function _validateArray(data, {context, customTypeMap, getData, path, template}, done) {
  const {length, maxLength, minLength} = template;
...
function _validateObject(data, {context, customTypeMap, getData, path, template}, done) {
  ...
  }, Object.entries(template.fields).map(([key, value]) => d => {
    _validate(data[key], {
      context,
      customTypeMap,
      getData,
...
function _validateVariant(data, {context, customTypeMap, getData, path, template}, done) {
  ...
  _callbackChain(err => {
  ...
  }, template.variants.map((variant, index) => d => {
    const {hint} = variant;
    ...
    _callbackChain(() => {
      ...
      d();
    }, filteredVariants.map(variant => dd => {
      _validate(data, {
        context,
        customTypeMap,
        getData,
...
function _validate(data, {context, customTypeMap, getData, path, template}, done) {
  const {
    defaultValue,
    isNullable,
    missingStrategy,
  ...
  _callbackChain(err => {
    ...
    _validateArray(newData, {context, customTypeMap, getData, path, template}, (err, result) => {
      if (err) {
        d(err);
    ...
    case 'object':
      if (newData) {
        _validateObject(newData, {context, customTypeMap, getData, path, template}, (err, result) => {
          if (err) {
            d(err);
    ...
    case 'string':
      ...
      return;
    case 'variant': {
      _validateVariant(newData, {context, customTypeMap, getData, path, template}, (err, result) => {
        if (err) {
          d(err);
