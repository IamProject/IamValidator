 ...
 * @return {array} list of templates sorted according to hint function results
 */
function sortedVariants(variants, data) {
  return variants.slice(0)
    .map(variant => ({
      variant,
      matchesHint: (typeof variant.hint === 'function') ? Number(Boolean(variant.hint(data))) : 0
    }))
 ...
 * @throws IamValidatorError
 */
function validateInternal(TEMPLATE, data, path, options) {
...
function validate(TEMPLATE, data, path, options) {
  ...
  if (isArray || (TEMPLATE.type === 'variant')) {
    const variants = sortedVariants(isArray ? TEMPLATE : TEMPLATE.variants, data);

    for (let i = 0; i < variants.length - 1; ++i) {
      try {
        return validateInternal(variants[i], data, path, options);
      } catch (err) {
        //
      }
    }
    return validateInternal(variants[variants.length - 1], data, path, options);
  }

  return validateInternal(TEMPLATE, data, path, options);
}
