 ...
 * @param {object} options global options for validation
 * @param {*} field the tesing field value
...
/**
 * Tests null object's permission to be null
 * @param {object} TEMPLATE template that optionally contains isNullable
 * @param {null} data the null object
 * @param {string} path path to the null object in parent
 * @return {null} the null object
 * @throws IamValidatorError
 */
function validateNull(TEMPLATE, data, path) {
...
function validate(TEMPLATE, data, path, options) {
  ...
  if (type !== TEMPLATE.type && type !== "null") {
  ...
  if (Array.isArray(TEMPLATE.values)){
    ...
    if (!inValues){
      throw new IamValidatorError(CODES.NOT_IN_VALUES, { path: path, src: data });
    }
  }
...
module.exports = class IamValidator {
  ...
  static validate(template, data, options) {
    return validate(template, data, '_root', options);
  }
};
