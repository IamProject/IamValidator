...
function validateArray(TEMPLATE, data, path, options = {}) {
  ...
  return data.map((item, index) => {
    return validate(TEMPLATE.element, item, _createPath(options.arrayPathMode, index, path), options);
  });
}
...
function validate(TEMPLATE, data, path, options) {
  ...
  if (isArray || (TEMPLATE.type === 'variant')) {
    ...
    if (!Array.isArray(variants)) {
      return validateInternal(variants, data, path, options);
    }
...
module.exports = class IamValidator {
  ...
  validate(data, options = {}) {
    ...
    return validate(this.TEMPLATE, data, path, options);
  }
  ...
  static validate(template, data, options = {}) {
    const path = options.arrayPathMode ? (options.path || ['_root']) : (options.path || '_root');
    return validate(template, data, path, options);
  }
};
